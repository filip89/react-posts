...
export const getPostRequest = (postId: number) => {
    return () =>
        batchGETRequests(urls.post(postId), urls.postComments(postId), urls.users).then(([post, comments, users]) =>
...
export const getPostsRequest = () => {
    return () =>
        batchGETRequests(urls.posts, urls.comments, urls.users).then(([post, comments, users]) =>
